...
from .config import BINDER_FAMILIES, FAMILY_PROTOTYPES, USE_FIXED_WATER, WATER_FIXED
...
def build_models(df) -> ModelsBundle:
    ...
    df_f7 = df.dropna(subset=["Strength7d"]).copy()
    knn_f7_global = KNeighborsRegressor(n_neighbors=4, weights="distance").fit(
        df_f7[["Water/Binder", "_slag_frac", "_fly_frac"]].values, df_f7["Strength7d"].values
    )

    # --- water models (3d) ---
    family_models_water_3d: Dict[str, KNeighborsRegressor] = {}
    for fam in BINDER_FAMILIES.keys():
        sub = df[df["_family"] == fam].dropna(subset=["Strength3d", "Strength28d", "Water/Binder", "Free Water"])
        if len(sub) >= 2:
            family_models_water_3d[fam] = KNeighborsRegressor(n_neighbors=3, weights="distance").fit(
                sub[["Strength3d", "Strength28d", "Water/Binder"]].values,
                sub["Free Water"].values
            )

    df_w3 = df.dropna(subset=["Strength3d", "Strength28d", "Water/Binder", "Free Water"])
    ...
    family_models_water_7d: Dict[str, KNeighborsRegressor] = {}
    for fam in BINDER_FAMILIES.keys():
        sub = df[df["_family"] == fam].dropna(subset=["Strength7d", "Strength28d", "Water/Binder", "Free Water"])
        if len(sub) >= 2:
            family_models_water_7d[fam] = KNeighborsRegressor(n_neighbors=3, weights="distance").fit(
                sub[["Strength7d", "Strength28d", "Water/Binder"]].values,
                sub["Free Water"].values
            )

    df_w7 = df.dropna(subset=["Strength7d", "Strength28d", "Water/Binder", "Free Water"])
    ...
    return ModelsBundle(
        family_curves=family_curves, A_g=A_g, b_g=b_g,
        wb_inv_models=wb_inv_models, wb_ranges=wb_ranges,
        family_models_f3=family_models_f3, knn_f3_global=knn_f3_global,
        family_models_f7=family_models_f7, knn_f7_global=knn_f7_global,
        family_models_water_3d=family_models_water_3d, knn_water_global_3d=knn_water_global_3d,
        family_models_water_7d=family_models_water_7d, knn_water_global_7d=knn_water_global_7d,
...
def get_water(models: ModelsBundle, early_strength: float, f28: float, wb: float, fam_key: str, early_age_days: int = 3) -> float:
    if USE_FIXED_WATER:
